...
const CartContextProvider = ({ children }) => {
  ...
  // Load initial cart state from local storage or use initialState
  ...
  // Placeholder logic for cart operations
    ...
    dispatch({
      ...
      payload: state.cart.map((product) =>
        product._id === productId
          ? {
              ...product,
              qty: type === "increment" ? product.qty + 1 : product.qty - 1,
            }
          : product
      ),
    });
  ...
  const deleteProductFromCart = async (productId) => {
    ...
    dispatch({
      ...
      payload: state.cart.filter((product) => product._id !== productId),
    });
  };
  ...
  // Calculate total and actual prices of the cart
